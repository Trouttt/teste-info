...
class UpdateVehicleService {
  public async execute({
    ...
    try {
      ...
      const verifyPlateIsRegistered = await vehiclesRepository.findVehicleByPlate(
        plate,
      );
      ...
      if (verifyPlateIsRegistered) {
        throw new AppError('Placa não disponível!!!', 400);
      }

      if (verifyChassiIsRegistered) {
        throw new AppError('Chassi não disponível!!!', 400);
      }

      if (verifyRenavanIsRegistered) {
        throw new AppError('Renavam não disponível!!!', 400);
      }
